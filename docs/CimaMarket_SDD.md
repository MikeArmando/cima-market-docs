...
| Método | Ruta                                | Acceso                              | Propósito                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------ | ----------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/products`                     | Pública                             | Catálogo paginado con soporte de filtros (`campus`, `category`, `min_price`, `max_price`, `min_rating`, `lat`/`lng`), ordenamiento y búsqueda full-text (`q`) (→ RF-CAT-02, RF-CAT-03, RF-CAT-04). `min_rating` filtra sobre `users.average_rating` del vendedor. Cada publicación incluye el campo derivado `selling_now: boolean` del vendedor. Si `seller_id` está presente, `status` no se restringe a `active`. Devuelve solo registros con `deleted_at IS NULL`. |
...
- **`selling_now` en `GET /api/products`:** se calcula por publicación con el mismo `EXISTS (SELECT 1 FROM seller_live_locations WHERE seller_id = :sellerId AND expires_at > NOW())` que `GET /api/users/:id` (→ §4.2), evaluado sobre el mismo `seller_id` que la query ya joinea contra `users` para resolver `min_rating` y los datos del vendedor del listado. No implica una consulta adicional por publicación ni por vendedor.
...
| Coordenadas geográficas serializadas como `{lat, lng}` en respuestas de API | Toda columna `GEOMETRY(POINT)` expuesta públicamente (`campus_buildings.location`, `seller_live_locations.location`) se transforma de `{x, y}` (convención interna PostGIS/GeoJSON) a `{lat, lng}` antes de responder al cliente — consistente con `campuses.center_lat`/`center_lng` y con el orden que espera Leaflet.js (RF-MAP-01). Evita el error más común de datos geoespaciales: lat/lng invertidas. |
